                ...
                {isSearching && !isTyping && searchMobile && (
                  <div className="flex items-center justify-between text-sm">
                    <span className="text-gray-300">
                      Results for:{" "}
                      <span className="font-medium">{searchMobile}</span>
          ...
          <table className="w-full border border-gray-200 rounded-lg">
            <thead className="bg-gray-100 text-left">
              <tr>
                ...
                <th className="p-3">End Date</th> {/* ✅ NEW */}
            ...
            <tbody>
              {shops.map((shop) => {
                ...
                return (
                  <tr
                    key={shop._id}
                    className="border-t text-white hover:bg-gray-800/20 transition"
                  >
                    <td className="p-3 font-medium">{shop.shopName}</td>
                    <td className="p-3">
                      {shop.place}, {shop.state}
                    </td>
                    <td className="p-3">{shop.mobileNumber || "—"}</td>
                    ...
                    {/* PLAN */}
                    <td className="p-3">
                      {isActive ? getPlanName(shop.subscription?.plan) : "—"}
                    </td>
{/* END DATE */}
<td className="p-3">
  {shop.subscription?.isActive ? (
    <span className="text-sm text-yellow-300">
      {formatDate(shop.subscription?.endDate)}
    </span>
  ) : (
    "—"
  )}
