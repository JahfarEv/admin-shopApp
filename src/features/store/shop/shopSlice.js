  ...
  reducers: {
    ...
    filterShops: (state) => {
      ...
      state.filteredShops = state.shops.filter(shop => 
        ...
        shop.mobileNumber?.toString().includes(keyword) ||
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(deleteShop.fulfilled, (state, action) => {
        state.loading = false;
        state.shops = state.shops.filter(shop => shop._id !== action.payload);
        state.filteredShops = state.filteredShops.filter(shop => shop._id !== action.payload);
      })
      .addMatcher(
        (action) => action.type.endsWith('/pending'),
        (state) => {
...
export const { setSearchKeyword, clearError, filterShops } = shopSlice.actions;
export default shopSlice.reducer;